// DIYAT_V4_FLXR4d_dev.scad
// Drew Browning 2020
// licensed under the Creative Commons - Attribution - Non-Commercial - Share Alike license.

// part to build 
_part = "base";// [base,base-for-strap,retainer,button_convex,button_concave]
// outside diameter in mm (35 - 65)
_outsideDiameter = 35; // [35:65]
// activation-force (radial flexure thickness)
_force=.4; // [.4:Low, .7:Med, 1:High]
// LilyPad_PCB thickness: Thin=.8mm, Thick=1.6mm
_LilyPad_PCB = "Thin"; // ["Thin","Thick"]
...
module make($fn=_resolution) {
	...
	if (_part == "base") {
        ...
        *translate([0,-outsideDiameter*.18,baseHeight-2.3]){rotate([90,0,0]){
		...
		makeBase(outsideDiameter,baseHeight,_topBottomThickness,_lipHeight,_partGap,_LilyPad_PCB);
	} else if (_part == "base-for-strap") {
		makeBaseForStrap(outsideDiameter,baseHeight,_topBottomThickness,_lipHeight,_partGap,_LilyPad_PCB);
		...
		makeButton_cv(outsideDiameter,retainerHeight,_topBottomThickness,_lipHeight,_partGap,_force);
	} else if (_part == "button_concave") {
		makeButton_cc(outsideDiameter,retainerHeight,_topBottomThickness,_lipHeight,_partGap,_force);
        ...
        difference(){
            union(){
                ...
                *color("red"){
                ...
                color("Blue"){rotate([0,0,90]){translate([0,0,baseHeight+2.05]){//_outsideHeight-9.4
                    makeButton_cv(outsideDiameter,retainerHeight,_topBottomThickness,_lipHeight,_partGap,_force);
                    }}}
                *color("Blue"){rotate([0,0,90]){translate([0,0,baseHeight+2.05]){//_outsideHeight-9.4
                    makeButton_cc(outsideDiameter,retainerHeight,_topBottomThickness,_lipHeight,_partGap,_force);
            ...
            *translate([-outsideDiameter/2-1,0,_outsideHeight/2]){
                cube([outsideDiameter+2,outsideDiameter+14,_outsideHeight+5],true);}//YZ cross-section cube
        ...
        *translate([0,-outsideDiameter*.18,baseHeight-2.3]){rotate([90,0,0]){
            color("black") cylinder(_outsideDiameter*.35,1.5,1.5,false);}}
       // Lily_Tact
		//translate([0,0,_interiorHeight-.7-_partGap]){rotate([90,0,90]){color("plum"){
		translate([0,0,_interiorHeight-1.5-_partGap]){rotate([90,0,90]){color("plum"){
			if (_LilyPad_PCB == "Thick") {
				import("../TactSwitches/SparkFun_LilyPad-Button1.6mmPCB3.stl");
			} else {
				translate([0,.8,0]){import("../TactSwitches/SparkFun_LilyPad-Button.8mmPCB.stl");}
			}
		}}}
        //test thread gap
...
module makeBase(diameter, height, base, lipHeight, partGap, LilyPad_PCB) {
	height = max(height, base+lipHeight);
	radius = diameter/2;
	innerRadius = radius - 5.6 - partGap;
	fullHeight = height + lipHeight;
	rounding = 2.0;
    //standoff height dependent on LilyPad PCB height
	standoff = LilyPad_PCB == "Thick" ? height-1 : height-.2;//conditional: if LilyPad_PCB == "Thick", then standoff=height-8.3, else standoff=height-7.5; -1,-.2
	post = LilyPad_PCB == "Thick" ? 2.2 : 1.4;//conditional: if LilyPad_PCB == "Thick", then post=2.2, else post=1.4; -1,-.2
    translate([0,0,0]){union(){
        ...
        translate([0,0,-2-partGap]){union(){
            ...
            translate([-4.2,0,7.5]){cylinder(2.5,r=1.25,true);}//post0
            translate([4.2,0,7.5]){cylinder(2.5,r=1.25,true);}//post1
        }}
    }}
}
module makeBaseForStrap(diameter, height, base, lipHeight, partGap, LilyPad_PCB){
	height = max(height, base+lipHeight);
	radius = diameter/2;
	innerRadius = radius - 5.6 - partGap;
	fullHeight = height + lipHeight;
	rounding = 2.0;
    //standoff height proportioal to LilyPad PCB height
	standoff = LilyPad_PCB == "Thick" ? height-1 : height-.2;//conditional: if LilyPad_PCB == "Thick", then standoff=height-8.3, else standoff=height-7.5; -1,-.2
	post = LilyPad_PCB == "Thick" ? 2.2 : 1.4;//conditional: if LilyPad_PCB == "Thick", then post=2.2, else post=1.4; -1,-.2
    translate([0,0,0]){union(){
        difference(){
    ...
    rotate([0,0,90]){
        difference(){
            ...
            *translate([0,0,1]){cube([diameter+10,32,2],true);}//test rectangular base plate
    ...
    *color("Black"){
        union(){
            translate([0,-diameter-radius-2.5,2.4]){cube([25.4,diameter*2,.75],true);}
            difference(){
                translate([0,-radius-2.5,1.75]){rotate([0,90,0]){cylinder(r=1,h=25.4,center=true);}}
                translate([0,-radius-4,2]){cube([25.6,3,3],true);}
            }
        }
        rotate([0,0,180]){
            translate([0,-diameter-radius-2.5,2.4]){cube([25.4,diameter*2,.75],true);}
    ...
    // screws (M3 counter sunk)
    union(){
        translate([13,(radius+2),1.6]){
            difference(){
                color("Black"){translate([0,0,0]){cylinder(r=1,r2=2,h=1,center=true);}}//right}
                translate([0,0,.5]){cube([.5,2.5,1],true);}
                translate([0,0,.5]){cube([2.5,.5,1],true);}
            }
        }
        translate([-13,(radius+2),1.6]){
            difference(){
...
module makeButton_cv(diameter, height, base, lipHeight, partGap, force){
	height = max(height, base+lipHeight);
	radius = diameter/2;
	innerRadius = radius-3.1-partGap;
	rounding = 1.5;
	buttonRadius = innerRadius-6.25;
	buttonArea = buttonRadius*buttonRadius*PI;
	translate([0,0,6]){rotate([0,180,0]){
        for (i = [0:45:337.5]){
            rotate([0, 0, i])
            makeArcDB(_outsideDiameter,force);
        }
        difference(){
            union(){
                // convex button
...
module makeButton_cc(diameter, height, base, lipHeight, partGap, force){
	height = max(height, base+lipHeight);
	radius = diameter/2;
	innerRadius = radius-3.1-partGap;
	rounding = 1.5;
        ...
        difference(){
            union(){
                ...
                difference(){
                    ...
                    // concavity
                    if (diameter > 54) {translate([0,0,-diameter/(cos(diameter)*2)+6]){sphere(r=diameter/(cos(diameter)*2));}//
                    } else if(diameter <= 54 && diameter > 44){translate([0,0,-diameter/(cos(diameter)*2.025)+5]){sphere(r=diameter/(cos(diameter)*2));}//
                    } else if(diameter <= 44 && diameter > 39){translate([0,0,-diameter/(cos(diameter)*2.05)+4.1]){sphere(r=diameter/(cos(diameter)*2));}//
                    } else if(diameter <= 39){translate([0,0,-diameter/(cos(diameter)*2.075)+3.4]){sphere(r=diameter/(cos(diameter)*2));}//
					}
                }
                // button lip
                translate([0,0,5]){cylinder(r=innerRadius+.35, h=1);}//rim
            }
            //rim-slot-cutter (space for flexures)
            difference(){
                translate([0,0,4]){cylinder(r=innerRadius-2.2, h=3);}//OD of cutter
                translate([0,0,4]){cylinder(r=innerRadius-5, h=5);}//ID of cutter
            }
...
module makeArcDB(diameter,gf){
	radius = diameter/2;
    rotate([0,180,0]){translate([3.7,0,-8.45]){// 3.3, -8.45
            difference(){
                translate([0,0,2.45]){cylinder(r=radius-4.6, h=gf);}//OD of flexure, h=.4 to 1
                //translate([0,0,0]){cylinder(r=radius-(diameter*.02+4.4), h=5);}//ID of flexure cutter
                translate([0,0,0]){cylinder(r=radius-(diameter*.02+4.4), h=5);}//ID of flexure cutter (+4.4 to 4.7)
                translate([2,-radius,0]){cube([diameter,diameter,10]);}//rectangular cutter
                translate([-radius,-diameter,0]){cube([diameter,diameter,10]);}//rectangular cutter
            }
            //test radial flexure width
			//color("red"){translate([radius-11.5,-5.5,1.5]){cylinder(r=.25, h=1.5);}}//test
    }}
}
module torus(r1, r2) {
